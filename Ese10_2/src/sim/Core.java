...
public class Core {
	...
	private int findAvailableTable(int numPersone) {
		...
		int postiTavoloMin = 0;
		int piuPiccoloId=-1;
		boolean primoStep = true;
		...
		for(Table tb: tavoliList){
						...
						if(tb.isLibero()&&tb.getNumPostiASedere()>= numPersone){
									...
									if(primoStep){
										postiTavoloMin = tb.getNumPostiASedere();
										primoStep=false;
									}
									
									if(tb.getNumPostiASedere()<=postiTavoloMin){
									piuPiccoloId = tb.getIdTable();
									postiTavoloMin = tb.getNumPostiASedere();
									}
						}
		}
	...
	public Table trovaTavolo(int idCustomers){
		List<Table> listaTavoli = new LinkedList<Table>(tavoli.values());
		Table eccolo = null;
		for(Table t: listaTavoli){
